...
const executeJavaScript = (code: string): Promise<{ output: string; error: string }> => {
...
const CodeEditor: React.FC<CodeEditorProps> = ({ darkMode, setDarkMode }) => {
  ...
  const currentFile = currentProject?.files.find(f => f.id === currentFileId) || currentProject?.files[0];
  ...
  const runCode = async () => {
    ...
    try {
      ...
      if (currentFile.language === "js" && useLocalExecution) {
        ...
        setError(
          err.response?.data?.error || "Execution failed. Please check your code."
        );
  ...
  const createNewFile = () => {
    ...
    const newFile: CodeFile = {
      ...
      content: DEFAULT_CODE_TEMPLATES[newFileLanguage as keyof typeof DEFAULT_CODE_TEMPLATES] || "",
    };
  ...
  const resetToTemplate = () => {
    ...
    if (window.confirm("Reset this file to the default template?")) {
      const template = DEFAULT_CODE_TEMPLATES[currentFile.language as keyof typeof DEFAULT_CODE_TEMPLATES] || "";
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold dark:text-white">Online Code Editor</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-4">
          ...
          <div className="space-y-2">
            {projects.map(project => (
              <div key={project.id} className="relative">
                ...
                >
                  <div className="flex items-center space-x-2">
                    <FolderOpen size={18} className={project.id === currentProjectId ? "text-blue-600 dark:text-blue-400" : "dark:text-white"} />
                ...
                {project.id === currentProjectId && (
                  <div className="ml-4 mt-2 space-y-1">
                    {project.files.map(file => (
                      ...
                      >
                        <div className="flex items-center space-x-2">
                          <File size={16} className={file.id === currentFileId ? "text-blue-600 dark:text-blue-400" : "dark:text-white"} />
        ...
        <div className="lg:col-span-3 space-y-4">
              ...
              <div className="flex items-center space-x-2">
                <span className="font-mono text-sm dark:text-white">
                  ...
                  <span className="ml-2 text-xs text-gray-500 dark:text-gray-400">
                    ({languageOptions.find(l => l.value === currentFile?.language)?.label || "Unknown"})
                  </span>
                </span>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4">
            <h3 className="text-lg font-semibold mb-2 dark:text-white">Output</h3>
            {output && (
      ...
      {showFileModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-96">
            <h2 className="text-xl font-bold mb-4 dark:text-white">Create New File</h2>
            <div className="mb-4">
              <input
                type="text"
                value={newFileName}
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-96">
            <h2 className="text-xl font-bold mb-4 dark:text-white">Editor Settings</h2>
            ...
            <div className="mb-4">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                When enabled, JavaScript code will run directly in your browser without needing a server.
              </p>
            </div>
            ...
            <div className="mb-4">
              <label className="block dark:text-white mb-1">Editor Height</label>
              <div className="flex items-center">
                <input
                  type="range"
                  min="200"
